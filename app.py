...
class PredictionResult:
    ...
    Attributes:
        prediction: Predicted fabric consumption in yards
        unit: Unit of prediction (always 'yards')
        confidence_lower: Lower 90% confidence bound (yards)
        confidence_upper: Upper 90% confidence bound (yards)
...
class UnitConverter:
    """
    Unit Conversion Utility — yards ↔ cm/inches for fabric width inputs.

    Retains yards_to_meters / meters_to_yards for MockModel internal physics
    (garment base constants are defined in meters); all user-facing values
    are output in yards.
...
class ModelManager:
    ...
    def predict(
        ...
        model dispatch (XGBoost / RF / LR / LSTM / Ensemble), and empirical
        confidence interval computation are all handled here.
        All outputs are in yards.
        ...
        Returns:
            PredictionResult: Prediction in yards with confidence bounds,
        ...
        try:
            ...
            # Validate prediction — LinearRegression can extrapolate to negative/zero.
            # Clamp to a safe minimum rather than hard-failing, then log a warning
            # so users know the result was floored.
            if not np.isfinite(prediction_base):
                raise PredictionError(f"Non-finite prediction value: {prediction_base}")
            if prediction_base <= 0:
                logger.warning(
                    f"{model_name} predicted non-positive value ({prediction_base:.4f}); "                    "clamping to minimum (1.0 yd). Consider using Ensemble or XGBoost."                )
                prediction_base = 1.0

            # Model always trained on yards; MockModel also returns yards.
            # Production models: metadata unit is 'yards' — no conversion needed.
            # Demo MockModel: physics in meters, converted to yards inside predict().
            metadata_unit = self.models.get('metadata', {}).get('unit', 'yards')
            if self.mode == ProcessingMode.PRODUCTION and metadata_unit != 'yards':
                # Legacy meters model — convert output to yards
                prediction_yd = UnitConverter.meters_to_yards(prediction_base)
            else:
                prediction_yd = prediction_base
            ...
            ci_bounds = (self.models.get('metadata') or {}).get('models', {}).get(
            ...
            confidence_lower = max(prediction_yd * (1.0 - ci_frac), 0.001)
            confidence_upper = prediction_yd * (1.0 + ci_frac)
...
class DataGenerator:
    ...
    def generate_historical_data(n_samples: int = 500) -> pd.DataFrame:
        ...
        # Garment base consumption in yards at standard 160 cm width
        ...
        base_per_unit_yd = np.array([
            AppConfig.GARMENT_BASE_CONSUMPTION_YD[g] for g in garment_types
        ])
        base_per_unit_yd *= (STANDARD_WIDTH / width_cm)
        base_per_unit_yd *= np.array([COMPLEXITY_MULT[c] for c in complexities])

        planned_bom_yd = order_quantities * base_per_unit_yd * BOM_BUFFER
        ...
        actual_yd = (
            ...
            * (1.0 + noise)
        ).clip(planned_bom_yd * 0.85, planned_bom_yd * 1.15)
        ...
        data = {
            ...
            'Planned_BOM_yards':         planned_bom_yd,
            'Actual_Consumption_yards':  actual_yd,
        }

        df = pd.DataFrame(data)
        df['Variance_yards']             = df['Actual_Consumption_yards'] - df['Planned_BOM_yards']
        df['Variance_%']                 = (df['Variance_yards'] / df['Planned_BOM_yards']) * 100
...
class UIHelpers:
    ...
    @staticmethod
    def render_footer(is_production: bool) -> None:
        ...
        <div style='text-align: center; color: #7f8c8d; padding: 20px;'>
            ...
            <p style='font-size: 14px;'>
                ...
                📊 Active Unit: <strong>YARDS</strong>
            </p>
            <p style='font-size: 14px;'>
...
class DashboardPage:
    ...
    def render() -> None:
        ...
        st.markdown("### AI-Powered Material Planning | Unit: **YARDS**")
        ...
        total_waste_usd = (
            df_history['Variance_yards'].clip(lower=0)
        ...
        pct_over = (df_history['Variance_yards'] > 0).mean() * 100
        ...
        with col2:
            st.metric(
                ...
                f"{accuracy_pct:.1f}%",
                help=f"100% − avg |Variance%| across {len(df_history):,} historical records"
            )
        with col3:
            st.metric(
                "💰 Est. Waste Cost",
                f"${total_waste_usd:,.0f}",
                help="Total over-consumption cost across historical dataset at Cotton baseline rate"
            )
        with col4:
            st.metric(
        ...
        DashboardPage._render_charts(df_history)
        DashboardPage._render_statistics(df_history)

    @staticmethod
    def _render_charts(df: pd.DataFrame) -> None:
        """Render dashboard charts (yards)"""
        ...
        with col1:
            st.subheader("📈 Consumption Trends (yards)")
            ...
            fig.add_trace(go.Scatter(
                ...
                name='Planned (yards)', line=dict(color='blue', dash='dash')
            ))
            fig.add_trace(go.Scatter(
                x=daily_data['Date'], y=daily_data[actual_col],
                name='Actual (yards)', line=dict(color='red')
            ))
            fig.update_layout(height=350, hovermode='x unified',
                             yaxis_title='Consumption (yards)')
    ...
    def _render_statistics(df: pd.DataFrame) -> None:
        """Render dashboard statistics (yards)"""
        variance_col = 'Variance_yards'

        col1, col2 = st.columns(2)

        with col1:
            st.subheader("📊 Variance Distribution (yards)")
            fig = px.histogram(df, x=variance_col, nbins=50,
                              title="Variance Distribution",
                              labels={variance_col: 'Variance (yards)'},
        ...
        with col2:
            ...
            fig.update_layout(xaxis_title='Avg Variance (yards)')
            st.plotly_chart(fig, width='stretch')
...
class SinglePredictionPage:
    ...
    @staticmethod
    def render(model_mgr: ModelManager) -> None:
        """Render the single prediction page"""
        st.title("🎯 Single Order Prediction")
        st.markdown("### Enter order details | Output unit: **YARDS**")

        with st.form("prediction_form"):
            SinglePredictionPage._render_form_inputs()
            submit = st.form_submit_button("🔮 Predict Fabric Consumption",
                                          width='stretch')

        if submit:
            SinglePredictionPage._process_prediction(model_mgr)

    @staticmethod
    def _render_form_inputs() -> None:
        ...
        with col2:
            ...
            st.caption(f"= {fabric_width_cm:.1f} cm")
    ...
    @staticmethod
    def _process_prediction(model_mgr: ModelManager) -> None:
        ...
        try:
            ...
            # Fabric width always in inches (yards UOM)
            ...
            with st.spinner('🔄 Calculating prediction...'):
                result = model_mgr.predict(order_data, model_name)
            ...
            # Catches encoding-map or session-state key misses (e.g. missing model key)
            logger.error(f"KeyError in prediction pipeline: {e}\n{traceback.format_exc()}")
            UIHelpers.show_error("Prediction Error", f"Internal key error for key {e} — please refresh the page and try again.")
        except Exception as e:
            logger.error(f"Unexpected error in prediction: {e}\n{traceback.format_exc()}")
            UIHelpers.show_error("Unexpected Error", str(e) if AppConfig.DEBUG else "Please refresh the page and try again.")
    ...
    def _render_results(
        ...
        """Render prediction results (all values in yards)"""
        st.markdown("---")
        ...
        with col1:
            st.metric(
                label="🎯 Predicted (yards)",
                value=f"{result.prediction:.2f}",
                help="AI-predicted fabric requirement in yards"
            )

        with col2:
            # Garment-specific BOM: qty × garment_base_yd × 1.05 buffer
            garment_base_yd = AppConfig.GARMENT_BASE_CONSUMPTION_YD.get(
                order_input.garment_type, 1.641)
            bom_estimate = (order_input.order_quantity * garment_base_yd
                            * AppConfig.DEFAULT_BOM_BUFFER)

            delta_pct = ((result.prediction - bom_estimate) / bom_estimate * 100)
            st.metric(
                label="📋 Traditional BOM (yards)",
        ...
        with col3:
            ...
            st.metric(
                ...
                help=f"Based on ${fabric_cost:.4f}/yard"
            )
        ...
        with col4:
            ...
            if diff_fabric >= 0:
                ...
                savings_delta = f"−{diff_fabric:.2f} yd vs BOM"
            else:
                savings_label = "⚠️ ML Overage vs BOM"
                savings_delta = f"+{abs(diff_fabric):.2f} yd vs BOM"
            st.metric(
                label=savings_label,
        ...
        st.markdown("---")
        st.subheader("📈 Prediction with 90% Confidence Interval (yards)")
        ...
        for y_val, label in [
            (result.confidence_lower, f"Lower: {result.confidence_lower:.2f} yd"),
            (result.confidence_upper, f"Upper: {result.confidence_upper:.2f} yd"),
        ]:
        ...
        fig.add_trace(go.Scatter(
            ...
            text=[f"  {result.prediction:.2f} yd"],
        ...
        fig.add_shape(type='line', x0=0.4, x1=1.6, y0=bom_estimate,
                      y1=bom_estimate,
                      line=dict(color='#e74c3c', width=2, dash='dash'))
        fig.add_annotation(
            x=1.52, y=bom_estimate,
            text=f"Traditional BOM: {bom_estimate:.2f} yd",
        ...
        fig.update_layout(
            ...
            yaxis_title='Fabric Consumption (yards)',
            height=380,
            showlegend=True,
        ...
        with st.expander("📋 Full Prediction Breakdown", expanded=False):
            per_unit_yd = result.prediction / order_input.order_quantity if order_input.order_quantity else 0
            ...
            breakdown = {
                "Parameter": [
                    ...
                    "Operator Experience",
                    "—",
                    "AI Prediction (total, yards)",
                    "Per-unit consumption (yards)",
                    "Confidence Interval Lower (yards)",
                    "Confidence Interval Upper (yards)",
                    "Traditional BOM (total, yards) ¹",
                    "BOM vs AI Δ (%)",
                    "Fabric Cost ($/yard)",
                    "Estimated Total Cost (AI)",
                    "Estimated Total Cost (BOM)",
                    "Model Used",
                    "Timestamp",
                ],
                "Value": [
                    ...
                    "—",
                    f"{result.prediction:,.3f} yards",
                    f"{per_unit_yd:.4f} yd/unit",
                    f"{result.confidence_lower:,.3f} yards",
                    f"{result.confidence_upper:,.3f} yards",
                    f"{bom_estimate:,.3f} yards",
                    f"{delta_pct:+.2f}%",
                    f"${fabric_cost:.4f}/yard",
                    f"${estimated_cost:,.2f}",
            ...
            # BOM formula footnote
            st.caption(
                "¹ Traditional BOM = qty × garment base consumption × 1.05 safety buffer "
                "(industry standard — does not adjust for actual fabric width). "
                "AI prediction incorporates actual width, defect rate, operator experience, and season."
            )
...
class BatchPredictionPage:
    ...
    def render(model_mgr: ModelManager) -> None:
        ...
        st.markdown("### Upload CSV for multiple predictions | Output: **YARDS**")
    ...
    def _generate_predictions(
        ...
        """Generate predictions for every row — output in yards."""
        with st.spinner('🔄 Processing...'):
            try:
                model_name = EncodingMaps.MODEL_DISPLAY.get(model_choice, 'ensemble')

                # BOM baseline in yards
                garment_bases_yd = df['Garment_Type'].map(
                    AppConfig.GARMENT_BASE_CONSUMPTION_YD
                ).fillna(1.641)
                df['Traditional_BOM'] = (
                    df['Order_Quantity'] * garment_bases_yd * AppConfig.DEFAULT_BOM_BUFFER
                )

                # ── Real ML predictions row-by-row (yards output) ────────────
                predicted_yd_list = []
                ...
                for i, (_, row) in enumerate(df.iterrows()):
                    try:
                        ...
                        result = model_mgr.predict(order_data, model_name)
                        predicted_yd_list.append(result.prediction)
                    except Exception:
                        # Row-level fallback: garment-specific BOM formula (yards)
                        g_base_yd = AppConfig.GARMENT_BASE_CONSUMPTION_YD.get(
                            str(row.get('Garment_Type', 'T-Shirt')), 1.641)
                        predicted_yd_list.append(float(row['Order_Quantity']) * g_base_yd)
                    progress.progress(int((i + 1) / n * 100))
                progress.empty()
                df['Predicted'] = predicted_yd_list
    ...
    def _render_batch_results(
        ...
        """Render batch prediction results (yards)"""
        # Summary
        st.markdown("---")
        ...
        display_cols = ['Order_ID', 'Order_Quantity', 'Garment_Type', 'Predicted']
        ...
        # Download options
        BatchPredictionPage._render_download_buttons(df, model_choice, savings_pct)
    ...
    def _render_download_buttons(
        ...
        """Render download buttons for batch results (yards)"""
        st.markdown("---")
        col1, col2 = st.columns(2)
        ...
        with col1:
            ...
            st.download_button(
                ...
                data=csv_results,
                file_name=f"predictions_yards_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                mime="text/csv",
                width='stretch'
            )
    ...
    def _generate_summary_text(
        ...
        """Generate summary text for download (yards)"""
        summary = f"""
...
-----------
Total Orders: {len(df)}
Total Predicted: {df['Predicted'].sum():,.2f} yards
...
class PerformancePage:
    ...
    @staticmethod
    def render(model_mgr: "ModelManager" = None) -> None:
        """Main entry point — renders full performance analysis (yards)."""
        st.title("📈 Model Performance Analysis")
        st.markdown("### Comparing all 5 ML models + Traditional BOM baseline | **YARDS**")

        # ── Load metrics ────────────────────────────────────────────────────
        metrics_df, ensemble_weights, feature_importance, cv_data, bom_data, is_live = PerformancePage._load_metrics(model_mgr)
        ...
        with tab1:
            try:
                PerformancePage._tab_overview(metrics_df, is_live)
            except Exception as e:
                st.error(f"Overview tab error: {e}")

        with tab2:
            try:
                PerformancePage._tab_error_analysis(metrics_df)
            except Exception as e:
    ...
    def _load_metrics(model_mgr):
        ...
        """
        cf = UnitConverter.METERS_TO_YARDS  # always yards
        is_live = False
        ensemble_weights = {}
        feature_importance = {}
        cv_data = {}
    ...
    def _tab_overview(df, is_live):
        ...
        if not ens.empty and not bom.empty:
            ...
            with col3:
                st.metric("📐 Ensemble RMSE (yards)", f"{er['RMSE']:.1f}",
        ...
        display_df.columns = ["Model", "RMSE (yards)", "MAE (yards)", "R² Score", "MAPE %"]
        try:
            styled = (display_df.style
                .highlight_max(subset=["R² Score"], color="#c6efce")
                .highlight_min(
                    subset=["RMSE (yards)", "MAE (yards)", "MAPE %"],
                    color="#c6efce")
                .format({"RMSE (yards)": "{:.2f}",
                         "MAE (yards)":  "{:.2f}",
                         "R² Score": "{:.4f}",
                         "MAPE %":   "{:.2f}%"}))
    ...
    @staticmethod
    def _tab_error_analysis(df):
        st.subheader("📊 Error Metrics Breakdown (yards)")
        ...
        with col1:
            ...
            fig.update_layout(title="RMSE by Model (yards)",
                              yaxis_title="RMSE (yards)",
        ...
        with col2:
            ...
            fig.update_layout(title="MAE by Model (yards)",
    ...
    def _tab_ensemble(ensemble_weights, metrics_df):
        ...
        wt_rows = [{"Model": m, "Weight": f"{w:.4f}",
                   ...
                   for m, w in sorted(disp_w.items(),
                                      key=lambda x: x[1], reverse=True)]
        st.dataframe(pd.DataFrame(wt_rows), width='stretch')
...
class ROICalculatorPage:
    ...
    @staticmethod
    def render() -> None:
        """Render the ROI calculator page (yards)"""
        st.title("💰 Economic Impact Calculator")
        st.markdown("### Calculate savings | Unit: **YARDS**")
        ...
        with col1:
            ...
            fabric_cost = st.number_input(
                "Fabric Cost ($/yard)",
                1.0, 50.0, AppConfig.DEFAULT_FABRIC_COST_PER_YARD, 0.5
            )
        ...
        ROICalculatorPage._render_financial_analysis(
            ...
            implementation_months, fabric_cost
        )
    ...
    def _render_financial_analysis(
        ...
        ROICalculatorPage._render_environmental_impact(
            current_annual_waste, ml_improvement, fabric_cost
        )
    ...
    def _render_environmental_impact(
        ...
        """Render environmental impact metrics (yards)"""
        st.markdown("---")
        st.subheader("🌍 Environmental Impact")

        fabric_saved = (current_annual_waste / fabric_cost) * (ml_improvement / 100)
        fabric_saved_m = fabric_saved * UnitConverter.YARDS_TO_METERS
        ...
        with col1:
            st.metric("🧵 Fabric Saved (yards)", f"{fabric_saved:,.0f}")
...
class DocumentationPage:
    ...
    @staticmethod
    def render(is_production: bool) -> None:
        ...
        with tab3:
            DocumentationPage._render_faq(is_production)
    ...
    def _render_quick_start() -> None:
        ...
        This system uses **yards** as the standard unit for all fabric measurements.

        **To change units:**
        1. Use the sidebar unit selector
        2. All outputs are in yards
        ...
        **Batch Processing:**
        ...
        - **Unit:** All values displayed in yards
        """)
    ...
    def _render_unit_guide() -> None:
        ...
        **Yard to Meter:**
        ...
        If fabric costs **$9.30/yard** (≈ $8.50/m):
    ...
    @staticmethod
    def _render_faq(is_production: bool) -> None:
        """Render FAQ section"""
        st.markdown(f"""
        ...
        **Q: What if my fabric width is in different units?**
        ...
        - **Active Unit:** YARDS
        - **Show Dual Units:** No (yards only)
        - **Production Mode:** {'Active' if is_production else 'Demo'}
        - **Conversion Factor:** 1 yard = 0.9144 m (ISO)
