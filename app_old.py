...
def page_dashboard(models, meta, prod):
    ...
    lstm = mm.get("lstm", {})
    xgb = mm.get("xgboost", {})
    rf = mm.get("random_forest", {})
    lr = mm.get("linear_regression", {})
    ens = mm.get("ensemble", {})
...
1. **{_lbl(best_n)}** — Best ({best_m.get('rmse', 0):,.1f} yd RMSE)
2. **{_lbl('ensemble')}** — 2nd ({ens.get('rmse', 0):,.1f} yd)  
3. **{_lbl('xgboost')}** — 3rd ({xgb.get('rmse', 0):,.1f} yd)
4. **{_lbl('random_forest')}** — 4th ({rf.get('rmse', 0):,.1f} yd)
5. **{_lbl('linear_regression')}** — Baseline ({lr.get('rmse', 0):,.1f} yd)
        """)
    with fi2:
        bom_rmse = bom.get("rmse", 0)
        lstm_rmse = lstm.get("rmse", 0)
        improvement = ((bom_rmse - lstm_rmse) / bom_rmse * 100) if bom_rmse else 0
        st.warning(f"""
**Critical Note: RMSE vs MAPE**

- BOM baseline has **{bom.get('mape', 0):.2f}% MAPE** (5% safety buffer)
- Best ML ({_lbl(best_n)}) has **{lstm.get('mape', 0):.2f}% MAPE**
- BUT: {_lbl(best_n)} has **lower RMSE** ({lstm_rmse:,.1f} vs {bom_rmse:,.1f} yd)
...
- Use when exact consumption matters
        """)
    with sg2:
        lstm_mape = lstm.get("mape", 0)
        lstm_ci = (lstm.get("ci_bounds", {}).get("ci_fraction", 0) * 100)
        st.info(f"""
**Best for Robustness**: LSTM or Ensemble
- Ensemble RMSE: {ens.get('rmse', 0):,.1f} yd
- LSTM CI: ±{lstm_ci:.1f}% of prediction
- LSTM recommended for production
        """)
    with sg3:
        lr_mape = lr.get("mape", 0)
        st.error(f"""
